...
import {
  BookOpen, GraduationCap, ShieldCheck, HeartHandshake, Check,
  Star, Award, Users, School, BookMarked, Lightbulb, Globe, Heart,
  CheckCircle2, Sparkles, Trophy, type LucideIcon,
} from "lucide-react";

const ICON_MAP: Record<string, LucideIcon> = {
  BookOpen, GraduationCap, ShieldCheck, HeartHandshake,
  Star, Award, Users, School, BookMarked, Lightbulb, Globe, Heart,
  CheckCircle2, Sparkles, Trophy,
};

type Feature = { icon: string; title: string; points: string[] };
type SectionData = { section_title: string; section_subtitle: string; features: Feature[] };
...
export default function FeaturesSection() {
  const [section, setSection] = useState<SectionData>(DEFAULT);
  ...
  useEffect(() => {
    supabase
      ...
      .single()
      .then(({ data }) => {
        if (data) setSection(data as SectionData);
      });
  }, []);
  ...
  return (
    <section className="py-8 md:py-12">
      <div className="text-center mb-12">
        <h2 className="text-3xl md:text-4xl font-extrabold text-gray-800">{section.section_title}</h2>
          ...
          return (
            <div key={index} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group cursor-default h-full flex flex-col items-center">
              <div className="w-16 h-16 rounded-full bg-green-50 flex items-center justify-center mb-5 group-hover:bg-green-600 transition-all duration-300 relative overflow-hidden shrink-0">
                ...
                <IconComponent className="h-8 w-8 text-green-600 group-hover:text-white transition-colors duration-300 relative z-10" />
              </div>
