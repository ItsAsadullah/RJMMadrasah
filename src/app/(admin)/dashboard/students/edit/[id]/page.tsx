...
export default function AdminStudentEdit() {
  ...
  const checkDuplicateStudent = async (e: React.FocusEvent<HTMLInputElement>) => {
    ...
    const { data, error } = await supabase
        ...
        .eq("birth_reg_no", regNo).neq("id", id); 
  ...
  const downloadIDCard = () => {
    ...
    if(ctx) {
        ...
        const link = document.createElement('a'); link.download = `ID_${studentIdForDisplay}.jpg`; link.href = canvas.toDataURL("image/jpeg"); link.click();
    }
  };
  ...
  if (isLoadingData) return <div className="p-10 flex justify-center"><Loader2 className="animate-spin w-10 h-10 text-green-600" /></div>;
return (
    <div className="max-w-6xl mx-auto space-y-6 pb-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="flex justify-end pt-6">
           ...
           >
              {loading ? <><Loader2 className="animate-spin mr-2" /> প্রসেসিং...</> : <><Save className="mr-2" /> তথ্য আপডেট করুন</>}
           </Button>
        </div>
      </form>
                    ...
                    <div className="px-6 pb-6 pt-0 relative">
                        ...
                        <div className="bg-slate-50 rounded-lg p-3 space-y-2 border border-slate-100">
                            <div className="flex justify-between items-center border-b border-slate-200 pb-2">
                                ...
                                <span className="text-lg font-mono font-black text-slate-700">{studentIdForDisplay}</span>
                            </div>
